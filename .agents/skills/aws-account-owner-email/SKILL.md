...
> `.claude/skills/aws-account-owner-email/SKILL.md`, which is the **leading,
> authoritative** copy for this repo (see `CLAUDE.md` §"Tooling Conventions").
> Whenever the Claude Code version changes, port the same change here,
> translating Claude-specific mechanics to their Codex equivalent (e.g. Bash
> tool `dangerouslyDisableSandbox: true` ↔ `sandbox_permissions:
> "require_escalated"`). Never let this file diverge from the Claude Code copy.
