...
> `.claude/skills/e2evulnexception/references/error-patterns.md` are one skill
> kept in two harness trees — Codex reads this copy, Claude Code reads the
> other. Whichever copy an agent edits, the same change is ported to the other
> **in the same commit**; translate harness-specific mechanics rather than
> copying verbatim (e.g. `sandbox_permissions: "require_escalated"` ↔ Bash
> tool `dangerouslyDisableSandbox: true`). `.claude/skills/` is the tie-
