...
> **Sync policy (two-way, mandatory)**: This file and `.claude/skills/account-
> onboarding/SKILL.md` are one skill kept in two harness trees — Codex reads
> this copy, Claude Code reads the other. Whichever copy an agent edits, the same
> change is ported to the other **in the same commit**; translate
> harness-specific mechanics rather than copying verbatim (e.g.
> `sandbox_permissions: "require_escalated"` ↔ Bash tool
> `dangerouslyDisableSandbox: true`). `.claude/skills/` is the tie-breaker when
> the two disagree — that is a conflict rule, not a licence to edit one side
> only. Verify with `./scripts/check-skill-sync.sh` (exit 0) before calling the
