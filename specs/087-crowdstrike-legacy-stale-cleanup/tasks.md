...
- [X] T011 [US1] Extend `src/backendng/src/test/kotlin/com/secman/service/CrowdStrikeAssetCleanupServiceTest.kt` with five unit tests covering service-level flag gating, dedup, and response-field population. Note: scenarios (2)/(3)/(4) — manualCreator/scanUploader/COALESCE — are JPQL-fence behaviours and were placed in T013's integration test instead, where the actual SQL runs. The five service-level tests cover: (a) flag-on legacy candidate with LEGACY_NULL_TIMESTAMP reason; (b) flag-off ignored even if repo would return; (c) mixed batch with same id deduped (rule A wins); (d) mixed batch with distinct ids surfaces both reasons; (e) real run populates legacyDeletedCount per rule attribution
- [X] T012 [US1] Added four audit-service-level unit tests in `CrowdStrikeCleanupAuditServiceTest.kt` covering FR-010 override-vs-default resolution: override=true with default=false → forces on; override=false with default=true → forces off; override=null falls back to default in both directions
- [X] T013 [P] [US1] Created `src/backendng/src/test/kotlin/com/secman/repository/AssetRepositoryLegacyStaleTest.kt` extending `BaseIntegrationTest` with seven integration tests covering the JPQL fence: row passes fence → returned; manualCreator set → excluded; scanUploader set → excluded; non-CrowdStrike owner → excluded; non-stale → excluded; COALESCE fall-through (lastSeen NULL but updatedAt old → returned); countLegacyCrowdStrikeTotal returns full population minus the cutoff filter. Class declared `open` for Micronaut AOP/transaction proxying

### Implementation for User Story 1

- [X] T014 [US1] Add `findLegacyCrowdStrikeStale(ownerLiteral: String, cutoff: LocalDateTime): List<Asset>` and `countLegacyCrowdStrikeTotal(ownerLiteral: String): Long` to `src/backendng/src/main/kotlin/com/secman/repository/AssetRepository.kt` using `@io.micronaut.data.annotation.Query` with the JPQL from data-model.md §6. Verifies T013
- [X] T015 [US1] Update `cleanup()` in `src/backendng/src/main/kotlin/com/secman/service/CrowdStrikeAssetCleanupService.kt`: (a) change signature to accept `includeLegacy: Boolean` parameter (no default — caller resolves); (b) call `assetRepository.findLegacyCrowdStrikeStale(AssetOwners.CROWDSTRIKE_IMPORT, cutoff)` when `includeLegacy` is true; (c) tag rule-A candidates with `reason = TIMESTAMP_STALE`, rule-B with `LEGACY_NULL_TIMESTAMP`; (d) combine via `(timestampCandidates + legacyCandidates).distinctBy { it.id }`; (e) compute `legacyCandidateCount` and `legacyDeletedCount` from the result; (f) populate the new response fields. Verifies T011
- [X] T016 [US1] Update `run()` in `src/backendng/src/main/kotlin/com/secman/service/CrowdStrikeCleanupAuditService.kt` to accept an optional `includeLegacy: Boolean? = null` parameter, resolve `effective = override ?: configuredDefault` (inject the default via `@Value("${secman.crowdstrike.cleanup.include-legacy:false}")`), and forward to `cleanupService.cleanup(..., includeLegacy = effective)`. The scheduler call path passes no override so it always reads the configured default. Verifies T012
- [X] T017 [US1] Update `deleteNotSeenByCrowdStrike()` in `src/backendng/src/main/kotlin/com/secman/controller/AssetController.kt` to forward `request.includeLegacy` to `crowdStrikeCleanupAuditService.run(..., includeLegacy = request.includeLegacy)` (still passes `maxDeletePercent = null` per existing manual-run-no-brake convention; spec edge case + SC-004)
- [X] T018 [US1] Frontend: in `src/frontend/src/components/CrowdStrikeStaleAssetCleanup.tsx` — (a) added an "Include legacy CrowdStrike rows" toggle whose initial state comes from `config.includeLegacy` fetched from `/api/crowdstrike/cleanup/config` (SC-006); (b) request body sent to POST endpoint includes `includeLegacy`; (c) dry-run summary renders the split `candidates: N (timestamp: X, legacy: Y, deleted: Z)` only when toggle is ON, plain `candidates: N, deleted: M` when OFF (FR-012). **Also completed T022** (US3 history table column) since it edits the same file: added "Legacy (cand/del)" column rendering `legacyCandidateCount/legacyDeletedCount` with muted styling for 0/0 historical rows
...
- [X] T019 [US2] Created `CrowdStrikeCleanupAuditServiceIntegrationTest` with `testSafetyBrakeUsesWidenedDenominator`: seeds 5 timestamped (1 stale) + 20 legacy (none stale) and asserts the run does NOT abort (1/(5+20) = 4% < 10%). Without widening, this scenario would compute 1/5 = 20% and ABORT, exposing the bug. Assertions confirm `totalCrowdStrikeTracked = 25` post-run
- [X] T020 [P] [US2] Added three more integration tests in the same file: `audit row persists legacy counts on a SUCCESS run`, `audit row persists legacy counts on ABORTED_SAFETY_BRAKE`, and `manual run bypasses brake even when includeLegacy=true and blast radius is high` (analyze finding F4 / spec edge case + SC-004). The PARTIAL sub-case from the original task description is implicitly covered by the SUCCESS test's mechanics — true PARTIAL requires injecting a delete failure mid-run, which adds Mockk infrastructure beyond integration-test scope; service-level PARTIAL is already covered by `CrowdStrikeAssetCleanupServiceTest.reports per asset deletion failures...`

### Implementation for User Story 2

- [X] T021 [US2] Updated `CrowdStrikeCleanupAuditService`: (a) `checkSafetyBrake` numerator now adds rule-B candidate count via `findLegacyCrowdStrikeStale` when `includeLegacy=true`; (b) new `safeTotalCombined(includeLegacy)` widens the denominator to `countCrowdStrikeTracked + countLegacyCrowdStrikeTotal`; (c) `persistRun` signature gains `legacyCandidateCount` and `legacyDeletedCount` parameters and forwards to the entity; (d) all three call sites (success path, FAILED path, ABORTED_SAFETY_BRAKE path) populate the new fields with appropriate values (zero deletions for ABORTED; matched values for SUCCESS/PARTIAL; zero for FAILED). Brake-tripped log line now decomposes the numerator into timestamp + legacy components for ops visibility
...
- [X] T022 [US3] Frontend: in `src/frontend/src/components/CrowdStrikeStaleAssetCleanup.tsx`, added a "Legacy (cand/del)" column to the recent-runs table that renders `${run.legacyCandidateCount}/${run.legacyDeletedCount}` with `text-muted` styling for 0/0 rows (historical rows persisted before V210). Done as part of T018 since it's the same file edit. US3 acceptance scenarios verified manually via quickstart Step 5 (analyze finding F5 — no automated frontend test added)
