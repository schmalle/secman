...
**Detection**: Read `SECMAN_SSL_ACCEPT_ALL` from environment (resolved by `op run` from 1Password). Parse `true`/`1`/`yes` case-insensitively, matching the pattern in `./bin/secmanng`.
