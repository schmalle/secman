...
- [x] T015 [P] [US3] Add `NODE_TLS_REJECT_UNAUTHORIZED=0` conditional export in `tests/js-error-scanner.sh` — inside the `op run` subshell, detect `SECMAN_INSECURE` value (matching the `case` pattern from `./bin/secmanng` lines 26-29) and export the Node.js env var before invoking `node`
