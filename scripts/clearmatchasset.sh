...
export AWS_ASSET_BUCKET_NAME="pass://Test/SECMAN/AWS_ASSET_BUCKET_NAME"
export AWS_ASSET_BUCKET_KEY_NAME="pass://Test/SECMAN/AWS_ASSET_BUCKET_KEY_NAME"
...
# fails authentication (401) or fails bucket-name validation. The CLI reads
# AWS_ASSET_BUCKET_NAME / AWS_ASSET_BUCKET_KEY_NAME / SECMAN_ADMIN_NAME /
# SECMAN_ADMIN_PASS / SECMAN_INSECURE from the environment directly — pass-cli
# does resolve those correctly.
pass-cli run -- java -Xmx4g -Xms2g -jar ./src/cli/build/libs/cli-0.1.0-all.jar asset-match-clear --max-delete-percent 50 --verbose "$@"
