...
pass-cli run --env-file "$REPO_ROOT/secmanpp.env" -- java -Xmx4g -Xms2g -jar ./src/cli/build/libs/cli-0.1.0-all.jar query servers --device-type SERVER --severity CRITICAL,HIGH --min-days-open 1 --save --username $SECMAN_ADMIN_NAME --password $SECMAN_ADMIN_PASS --last-seen-days 14
