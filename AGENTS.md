...
Kotlin uses 4-space indentation, `UpperCamelCase` types, constructor injection, and immutable data classes. TypeScript/TSX sticks to 2-space indentation, named exports, and ESLint via `npm run lint`, ordering imports external, internal, then relative.
...
Commits use the `type(scope): description` convention (e.g., `feat(assets): add export`); keep subjects imperative and squash noisy WIP steps. Pull requests should include motivation, verification commands (`./gradlew build`, `npm run test:checkin`), and screenshots for UI changes. Flag schema changes, feature toggles, and manual data steps, linking issues or specs before review.
