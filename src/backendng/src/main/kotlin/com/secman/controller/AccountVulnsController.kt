...
open class AccountVulnsController(
    ...
    open fun getAccountVulns(authentication: Authentication): HttpResponse<*> {
        return try {
            ...
            // Estate too large to render in one page (see AccountVulnsService.MAX_RENDERED_ASSETS).
            // Reachable by ADMIN/SECCHAMPION, who see every asset.
            logger.warn("Account vulns refused for {}: {}", authentication.name, e.message)

            val error = ErrorResponse(
                message = e.message ?: "Too many assets to display.",
                status = HttpStatus.BAD_REQUEST.code
            )

            HttpResponse.status<ErrorResponse>(HttpStatus.BAD_REQUEST).body(error)

        } catch (e: IllegalStateException) {
            // Authentication carried no email; admins are no longer rejected here.
            logger.warn("Account vulns unavailable for {}: {}", authentication.name, e.message)
