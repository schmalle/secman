...
| `VULNERABILITIES_READ` | `get_vulnerabilities`, `get_all_vulnerabilities_detail`, `get_all_accessible_vulnerabilities`, `get_asset_most_vulnerabilities`, `get_overdue_assets`, `*_exception_request*`, `get_vulnerability_heatmap`, `refresh_vulnerability_heatmap`, `get_top_accounts_by_finding_age` |
| `ASSESSMENTS_READ` | `list_aws_account_risk_assessments` (ADMIN) |
...
- **`create_exception_request`** — `vulnerabilityId` (required unless `kind=NO_EDR`), `reason`* (50–2048 chars), `expirationDate`* (ISO-8601 future), two-axis `subject` (`ALL_VULNS|PRODUCT|CVE`) × `scope` (`GLOBAL|IP|ASSET|AWS_ACCOUNT|OS`), `subjectValue` (product pattern / comma-separated CVE list), `scopeValue` (IP, AWS account id, or — for `scope=OS` — an OS-version substring matched case-insensitively against `Asset.osVersion`, suppressing the CVE(s) on every matching asset), `assetId` (for `scope=ASSET`). Third axis `kind` (`VULNERABILITY` default | `NO_EDR`): **`NO_EDR`** records "this asset cannot run an EDR agent" — requires `scope=ASSET` + `assetId`, takes no `vulnerabilityId`/`subjectValue`/`scopeValue`, is stored with the filler `subject=ALL_VULNS`, **suppresses nothing**, and only removes the asset from the EDR-coverage KPI denominator. Unlike a real `ALL_VULNS` request it does *not* need ADMIN/VULN, but the caller must be able to access the named asset. ADMIN/SECCHAMPION are auto-approved.
...
- **`import_user_mappings`** — `mappings[]`* (≤1000), each `email`* + at least one of `awsAccountId` (12 digits) or `domain`. `dryRun`. Optionally starts a risk assessment for the owner of every brand-new AWS account the import introduces: `startRiskAssessment` (bool), `riskAssessmentUseCase` (required when it is true), `riskAssessmentDeadlineDays` (≥1, default `7`). Returns counts `created`, `createdPending` (user not yet exists), `skipped`, `errors[]` (strings), plus `newAccounts[]` (`awsAccountId`, `emails`) and `riskAssessments[]` (`awsAccountId`, `ownerEmail`, `riskAssessmentId`, `assessor`, `endDate`, `useCase`, `releaseVersion`, `requirementCount`, `error`). Runs the same code path as REST `POST /api/user-mappings/bulk` and CLI `manage-user-mappings import`, so brand-new-account detection is identical. Assessments are pinned to the ACTIVE requirements release; the call is rejected with `VALIDATION_ERROR` when none exists or it has no requirements for the use case. API-key permission: `USER_ACTIVITY`; ADMIN role checked in the tool. See `docs/AWS_ACCOUNT_RISK_ASSESSMENT.md`. Requires User Delegation.
...
- **`list_aws_account_risk_assessments`** (ADMIN) — lists the risk assessments auto-started for owners of newly discovered AWS accounts. Args: `awsAccountId`, `ownerEmail` (case-insensitive), `status` (e.g. `STARTED`), `limit` (1-100, default `20`). Returns `assessments[]` (`riskAssessmentId`, `awsAccountId`, `ownerEmail`, `useCase`, `releaseVersion` and `releaseName` — the version of the security requirements the assessment is measured against, `null` for assessments started before release pinning — `assessor`, `respondent`, `startDate`, `endDate`, `status`, `reminderTwoDaysSentAt`, `reminderOneDaySentAt`, `createdAt`) and `count`. Only import-triggered assessments are tracked; manually created ones never appear. API-key permission: `ASSESSMENTS_READ`; ADMIN role checked in the tool (`ADMIN_REQUIRED` otherwise). Requires User Delegation.
...
- **`send_exception_expiry_reminders`** (ADMIN) — emails each vulnerability exception owner (the exception's `createdBy` username, mapped to that user's email) when the exception's `expirationDate` falls exactly `days` days from today. One consolidated email per owner listing all of their expiring exceptions. A reminder is sent at most once per (exception, expiration date) pair — safe to run daily via cron/scheduler without re-notifying, and a later edit that extends the expiration date triggers a fresh reminder once it re-enters the window. Args: `days` (default `7`, min `1`), `dryRun` (default `false` — preview recipients without sending). Result includes `status` (`SUCCESS`/`DRY_RUN`/`PARTIAL_FAILURE`/`FAILURE`), `exceptionsExpiring`, `ownersNotified`, `emailsSent`, `emailsFailed`, `recipients[]`, `failedRecipients[]`, `unmappedOwners[]` (owner usernames with no resolvable email), `alreadyNotified` (skipped — reminder already sent for that expiration date). API-key permission: `NOTIFICATIONS_SEND`. Mirrors CLI `send-exception-expiry-reminders`. Requires User Delegation.
- **`get_top_accounts_by_finding_age`** (ADMIN) — gets the AWS accounts whose oldest still-open, non-excepted vulnerability has been open the longest. Args: `limit` (1-50, default `10`). Returns `accounts[]` (`awsAccountId`, `accountName` — never null, falls back to the bare 12-digit account id when unnamed, `oldestFindingFirstSeenAt`, `oldestFindingDaysOpen`, `oldestFindingCve`, `oldestFindingSeverity`, `oldestFindingAssetName`, `oldestFindingAssetInstanceId`, `openFindingCount`, `affectedAssetCount`) and `count`. API-key permission: `VULNERABILITIES_READ`; ADMIN role checked in the tool (`ADMIN_REQUIRED` error otherwise). Backs `GET /api/admin/account-finding-age/top` and CLI `send-account-finding-age-report`. Requires User Delegation.
...
- **`send_github_repo_alerts`** (ADMIN) — alerts GitHub repo owners whose open high+critical count has not decreased over the last `thresholdDays` days (baseline = newest snapshot at least that old; alert when current `> 0` and `>=` baseline). Args: `dryRun` (default `false`), `thresholdDays` (default `30`), `force` (default `false` — bypasses the non-decrease comparison and alerts every eligible owner with open high/critical alerts, including repos with no baseline snapshot yet), `onlyEmail` (optional — restricts the run to repos owned by this email address, case-insensitive). Result includes `reposAlerted`, `reposExcepted[]` (active `github_repo_alert_exception`), `unmappedRepos[]` (no `ownerEmail`), `reposSkippedInsufficientHistory[]`, `recipients[]`. API-key permission: `NOTIFICATIONS_SEND`. Mirrors CLI `alert-github-repo-owners`. Requires User Delegation. See `docs/GITHUB_REPOS.md`.
- **`list_github_owner_email_mappings`** (ADMIN/VULN/SECCHAMPION) — lists GitHub owner (org/user login) → default email mappings used to auto-fill `ownerEmail` on repos under that owner. No arguments. API-key permission: `VULNERABILITIES_READ`. Requires User Delegation.
- **`create_github_owner_email_mapping`** (ADMIN/VULN) — creates a mapping. Args: `owner`*, `email`*. 409 if the owner is already mapped. Immediately backfills `ownerEmail` on existing repos under that owner whose value is currently blank — a manually-set or previously auto-filled value is never overwritten. API-key permission: `VULNERABILITIES_READ`. Requires User Delegation.
- **`delete_github_owner_email_mapping`** (ADMIN/VULN) — deletes a mapping by `id`*. Does not un-set any `ownerEmail` it previously backfilled. API-key permission: `VULNERABILITIES_READ`. Requires User Delegation.
- **`discover_github_owner_email_mappings`** (ADMIN/VULN) — best-effort auto-discovery: for already-imported repos with no `ownerEmail`, queries the GitHub API's public profile field (`GET /users/{owner}`, works for orgs and users) and creates a mapping (with the usual backfill) for each owner that has one set. Only touches owners without an existing mapping; never calls GitHub's repo-listing API. Args: `dryRun` (default `false`) — preview without creating mappings. Result includes `discoveredMappings[]` (`owner`, `email`, `repoCount`), `ownersSkippedNoPublicEmail[]`. API-key permission: `VULNERABILITIES_READ`. Errors: `NO_GITHUB_CONFIG` when no active GitHub App configuration exists. Mirrors CLI `manage-github-owner-mappings discover`. Requires User Delegation. See `docs/GITHUB_REPOS.md`.
