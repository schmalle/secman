...
- MCP: `mcp/tools/ImportGithubReposTool.kt`, `SendGithubRepoAlertsTool.kt`, `ListGithubOwnerEmailMappingsTool.kt`, `CreateGithubOwnerEmailMappingTool.kt`, `DeleteGithubOwnerEmailMappingTool.kt`, `DiscoverGithubOwnerEmailMappingsTool.kt` (auto-registered as `@Singleton` `McpTool` beans; permissions in `McpToolPermissions.kt`)
