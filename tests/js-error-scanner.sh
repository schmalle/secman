...
# targeting the production host (pass://test/secman/SECMAN_HOST) when an e2e run
...
if [ "$NEEDS_OP" = true ]; then
  # Check pass-cli (Proton Pass) availability
  if ! command -v pass-cli &>/dev/null; then
    echo "ERROR: pass-cli (Proton Pass) is not installed or not in PATH."
