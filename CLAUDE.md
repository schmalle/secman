...
| User Mappings | `GET /api/user-mappings/{current,applied-history}`, `POST/PUT/DELETE /api/user-mappings[/{id}]`; `POST /api/user-mappings/link-workgroup-accounts` (links AWS accounts to the workgroup named `aws-<display name>`, creating it when missing — `docs/AWS_ACCOUNT_WORKGROUP_LINKING.md`) | ADMIN |
...
the OWASP and code-hygiene gate self-tests and the coverage report in one pass. It never starts the
...
`src/backendng/…/com/secman/{domain,controller,service,repository,config,dto,filter,mcp,relay}/` · `src/frontend/src/{pages,components,services,layouts}/` · `src/cli/…/com/secman/cli/{commands,service}/` · `src/shared/…/com/secman/crowdstrike/{client,dto,auth,model}/` (Gradle subproject shared by backendng + cli) · `src/relay/{cmd,internal,deploy}/` (Go). `application.yml` + `email-templates/` under `src/backendng/src/main/resources/`. Env: `docs/ENVIRONMENT.md`. Full tree: `docs/ARCHITECTURE.md` §File layout.
...
- **Vulnerability SLA anchored to remediation availability (2026-08-24)** — `secman.vulnerability.use-patch-publication-date` now defaults to **true** in both `application.yml` and `VulnerabilitySettings` (they had disagreed; `VulnerabilitySettingsDefaultTest` pins both). Age measures from when a fix became available, falling back to detection age per row. **One-way door**: `first_seen_at` is kept as `min(prior, new)`, so flipping the flag back does not restore prior anchors — a real revert needs `UPDATE vulnerability SET first_seen_at = NULL WHERE source = 'CROWDSTRIKE'` plus a full re-import. Expect overdue counts to jump on the first import after the change. `docs/CROWDSTRIKE_IMPORT.md` §Timestamp fix.
