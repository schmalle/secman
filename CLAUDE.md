...
- **Public standard download** — `GET /api/requirements/export/{docx,xlsx}` take `standard`/`standardId` + `release`/`releaseId` (`release=latest` → the ACTIVE release), all `IS_ANONYMOUS`, so `curl -OJ ".../export/docx?standard=IT%2FOT%20Security&release=latest"` works with no credential. A standard selects requirements through its **use cases** (`Standard.useCases` → `Requirement.usecases`); release-scoped exports filter frozen snapshots. Fails loudly rather than degrading: no ACTIVE release → 404 (never the live corpus), disagreeing `releaseId`/`release` → 400, a standard with no use cases → "no requirements" (never everything). New slim `GET /api/standards/public` (`{id,name}`) powers the picker logged out; `GET /api/standards` stays authenticated. See `docs/PUBLIC_STANDARD_DOWNLOAD.md`.
