...
- **New-AWS-account risk assessment** — a mapping import introducing an unseen account ID can auto-start an assessment for its owner, *pinned* to the single `ACTIVE` release so later requirement imports cannot change questions already asked (`ReleaseRequirementScopeService`). No ACTIVE release, or none tagged for the use case = 400 / CLI exit 2. `POST /api/user-mappings/bulk`, MCP `import_user_mappings` (+`startRiskAssessment`/`riskAssessmentUseCase`/`riskAssessmentDeadlineDays`) and `list_aws_account_risk_assessments`. Docs: `docs/AWS_ACCOUNT_RISK_ASSESSMENT.md`.
- **Longest-open findings by AWS account (ADMIN)** — ranks accounts by their oldest open, non-excepted vulnerability (`COALESCE(first_seen_at, scan_timestamp)`). `GET /api/admin/account-finding-age/top`, page `/account-finding-age`, MCP `get_top_accounts_by_finding_age`, CLI `send-account-finding-age-report` (ADMINs only, not REPORT). Names in `aws_account` (V245), editable via `PUT /api/admin/aws-accounts/{id}/name`.
- **Exception expiry reminders** — CLI `send-exception-expiry-reminders` / MCP `send_exception_expiry_reminders` (ADMIN) mail each exception's `createdBy` owner exactly `--days` (default 7) before expiry, one consolidated mail each. Idempotent per (exception, expiration date) via `vulnerability_exception_expiry_reminder` (V244) — cron-safe; a new expiration date re-arms it. No scheduler; `POST /api/cli/vulnerability-exception-expiry-notifications/send`.
