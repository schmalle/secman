...
- **`/secure-code` — OWASP Top 10 by construction, with a static gate that is actually tested** — the binding OWASP checklist in CLAUDE.md is ~60 bullets over ten categories, and the real failure mode is partial recall, not defiance. The new skill routes a change to the two or three categories it can actually violate *before* code is written, names the one existing control to reuse for each (`AssetFilterService`, `ExcelSanitizer`, the `NmapParserService` XXE block, `SlackClient.validateWebhookUrl`, …), then verifies three ways: `./scripts/owasp-check.sh` for the mechanical rules, a semantic re-read against `references/blind-spots.md` for the ones no grep can see, and `/security-review` on auth/crypto/upload/export/outbound diffs. The gate is **diff-scoped by default and includes untracked files** — a freshly generated controller is invisible to `git diff` — and splits findings into **BLOCK** (a line this change added) and **REVIEW** (pre-existing in a file you touched); `--all` reports 101 BLOCK across the repo, which is precisely why gating on the whole repo would be useless. Precision was tuned against the real corpus, not guessed: injection rules now require a query context *and* an interpolation (24 findings → 1) and the A09 rule looks for the secret's **value** reaching the log call rather than the word appearing in the message (119 → 6). `scripts/test/owasp-check-test.sh` (30 assertions) plants a vulnerable fixture per rule, asserts it fires at the right severity, then plants the approved pattern and asserts silence — it already caught an unsupported `\b` in awk that was silently downgrading every BLOCK to REVIEW, and caught the gate flagging its own fixtures. `/testsuite` runs the self-test in its fast tier. **OWASP: no category touched** (two offline scripts + Markdown; no endpoint, entity or data path). Full detail below.
