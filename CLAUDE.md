...
- **EOL matching for applications and frameworks: Java's nine distributions, the .NET family, and four rules against false positives** — the EOL dashboard showed operating systems only while `/products` plainly listed EOL `.Net Framework` and `.NET Core`. **No data source was missing and none was added**: `EolSubjectType.ASSET_PRODUCT` and `scanInstalledProducts` were already correct, but `installed_product` was **empty at scan time** — it is fed by CrowdStrike **Discover** via the separate `secman installed-products` CLI, not by the Spotlight vulnerability import that fills `vulnerability.vulnerable_product_versions` behind `/products`. Matching was broken in four ways, each proven against the live estate. **Java could not match at all**: endoflife.date publishes no generic `java` product but nine vendor distributions (`oracle-jdk`, `amazon-corretto`, `eclipse-temurin`, …) because their support windows differ, so the five curated aliases targeting `"java"` resolved to a key no product owned and were **dropped silently at index build time** — the documented behaviour for speculative targets, and exactly why it was invisible. Each spelling now maps to its real distribution, an unidentifiable build produces no finding, and the vendor disambiguates `OpenJDK Platform` (enabled by adding `Inc`/`Corporation`/`Ltd`/`GmbH`/`LLC` to the noise tokens). **Legacy Java versions**: `1.8.0.392` *is* Java 8 but segments to `[1,8,0,392]` and matched no cycle; now rewritten to `8.0.392`, Java-only, leading segment `1` only, failing closed. **Labelled cycles matched by list order**: every `windows` cycle carries an edition label (`10-1507`, `11-24h2-e-lts`) and cycle comparison keeps only the leading numeric run, so the Windows Server 2022 build `10.0.20348.3451` prefix-matched `10-1507` — **3,672 findings, 42 % of the total, calling supported servers EOL since 2017**. Labelled cycles are now skipped (`isNumericCycle`); Windows Server is unaffected, its cycles are plain numbers. **Distribution packages**: a packaging revision (`-6ubuntu14.4`, `.el8_10`, `.amzn2`, `~esm1`) or a maintainer vendor (`Ubuntu Developers <…>`) *proves* the distro owns the support contract — Canonical still supports nginx 1.18 — so both are rejected; Red Hat/Amazon Linux are deliberately not vendor-blocked (their versions carry `.el8`/`.amzn2`, and blocking them would kill `redhat-build-of-openjdk`). Also curated: `Microsoft Windows Desktop Runtime` is the .NET desktop runtime, which the shorter `microsoft windows` alias captured and reported as Windows 8; and `ASP .Net Core` → `dotnet` (alias is `asp net core` — tokenization strips the leading dot). Live estate: 304 → **4,492 findings** (3,386 EOL, 1,106 approaching), with the Windows, Node-library and distro-package false-positive classes eliminated and Oracle JDK 6/7/8/11 plus Corretto reporting for the first time. Nine new tests (EVM-015..023) pin every rule in both directions; the 14 pre-existing cases still pass. **OWASP: A04 touched — clean, 0 static findings** (pure function, no SQL/auth/IO/HTML surface). Unrelated but required to verify anything: `RequirementExportTemplateValidationServiceTest` referenced a non-existent `org.apache.poi.xwpf.usermodel.HeaderFooterType` (it is `org.apache.poi.wp.usermodel`), which had been failing `compileTestKotlin` and blocking the **entire** backend test suite since the requirement-export-template commit; with it fixed, `RequirementExportTemplateIntegrationTest` "deactivated and activated again" failed — but the **endpoint is correct and the test was wrong**: `@MicronautTest` wraps each test in a transaction, so the injected repository keeps one persistence context while the controller commits in its own, and the *second* `findById` returns the cached instance rather than the controller's write (proven by instrumenting: the response body said `ACTIVE`, the repository re-read said `INACTIVE`). Fixed with the `entityManager.clear()` pattern already used by `StatisticsAggregateIntegrationTest`. Backend suite green. Full detail below.
...
- **AWS account onboarding: welcome mail, direct and guided risk assessments** — the "new AWS account → mail the owner, start an assessment" path becomes three modes: `WELCOME_ONLY` (a templated welcome mail), `DIRECT` (plus an assessment for a use case the operator names — today's behaviour) and `GUIDED` (plus a one-time expiring link whose answers resolve through admin-configured rules into a **set** of use cases; the assessment is created on submit, scoped to the **union** of every matching rule). CLI (`--onboarding-mode`, `--welcome-email`, `--questionnaire-expiry-days` on `import` and `import-s3`), REST and MCP, all dry-runnable, plus `simulate-onboarding` / `simulate_account_onboarding`, which runs the *same* code path against a made-up account id and address rather than a mock — hence rate limited, audited, and stamping every simulated mail with "this is a test, sent by \<actor\>". **`startRiskAssessment=true` alone still resolves to `DIRECT` with no welcome mail** — byte-identical to before, reconciled in `AccountOnboardingMode.resolve`; combining it with a non-`DIRECT` mode is rejected, never guessed. The matcher sees only choice ids (a BOOLEAN question is two choices, no second code path), every matching active rule contributes and nothing wins (`priorityOrder` is display order only), the single `isDefault` rule is consulted **after** everything else fails, and a resolution with no requirements in the ACTIVE release is an error rather than an empty questionnaire — unmatched answers are **persisted** and the link stays usable, so an admin can add the missing rule and the owner's original link still works. Rules are normalized tables, not a `ruleJson` blob: that precedent exists for a recursive condition, this one is a flat AND over choice ids and the rule→use-case link needs real FK integrity. **The public questionnaire is the first genuinely unauthenticated write path here**: 256-bit `SecureRandom` token (not `UUID.randomUUID()` — this token *creates* an assessment), single use via a guarded UPDATE **claimed before** creation, 14-day expiry, a byte-identical 404 for malformed/unknown/expired/used/cancelled so it is no enumeration oracle, the account id masked to four digits and the owner email never returned, a new bounded fixed-window rate limiter with a dedicated failed-lookup bucket, `@Secured(IS_ANONYMOUS)` explicit on every method **plus** an `intercept-url-map` entry so a stale `secman_auth` cookie cannot 401 an owner out of their own link, and the token never logged, returned, printed, or minted by a dry run — all asserted. Unanswered invites get one reminder then lapse; **nothing is ever auto-created**, because an assessment scoped by a guess looks authoritative and is not. `createAssessment` now takes `useCases: Set<UseCase>` (`use_case_name` widened 255→1024 in V253) and `sendStartNotification` became `notifyAssessmentStarted` so the guided path reuses the identical mail. Five private helpers moved into `EmailTemplateRenderer` — with a **closed template allowlist**, since the basenames are operator config and interpolating one into `getResourceAsStream` would be a classpath-traversal read — and the twice-copied recipient regex into `EmailAddressValidator`. New: ADMIN-**or-SECCHAMPION** admin API and UI (a deliberate widening vs Demand Classification's read-both/write-ADMIN, with a coverage matrix showing which answer combinations dead-end *before* an owner hits one), `/onboarding/[token]`, `docs/ACCOUNT_ONBOARDING.md` (28 worked examples: every mode × dry-run × surface), and the `/account-onboarding` skill in both trees. **OWASP: A01/A02/A03/A04/A05/A07/A08/A09 touched — 0 BLOCK.** **The Kotlin and CLI halves are compile-unverified** — Java 21 only, no Gradle 9.7.0 distribution (services.gradle.org 403s through the agent proxy) and no Maven Central route. Frontend build + 149 unit tests + skill-sync are green.
