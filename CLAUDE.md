...
Triggered by `/e2eexception`, `/admin-asset-e2e`, `/e2ejs`, `/e2evulnexception`, `/e2eeol`, `/importtest`, `/crowdstrike-vuln-match` skills.
...
- **End-of-life lifecycle management: catalogue, matcher, owner mail, repository ranking** — `secman eol-sync` pulls a lifecycle catalogue (default **endoflife.date**: public, unauthenticated, ~350 products) and re-matches every system's OS and installed products plus every imported repo's Dependabot packages; `secman send-eol-notifications` mails account owners about software going EOL in the next 12 months; `/vulnerabilities/eol` answers "which software in my account / on my servers is EOL"; ADMIN/SECCHAMPION additionally get the top 10 repositories by distinct EOL components. The fetch runs **in the backend**, like `import-github-repos`, so the host allowlist and SSRF checks stay in one place. **The matcher never guesses**: an unparseable version or a version matching no cycle produces no finding, a version embedded in the name beats the version field (Windows Server's build number matches no cycle; the `2019` in its name does), and a dateless EOL flag is EOL with a *null* horizon so it can never be counted into "within N months". Repository components use the Dependabot range's upper bound — a sound under-approximation, `EOL` only. Findings are replace-per-run by `scan_run_id` with an explicit delete and **no cascade from `Asset`**. Reads bind the caller's accessible asset ids; out-of-scope ids answer 404. **OWASP: A01/A03/A04/A05/A08/A09/A10 touched — 0 BLOCK, 35 REVIEW, all reviewed.** **Kotlin is compile-unverified** (no Gradle distribution or Maven Central route in that session); frontend build + 99 unit tests + skill-sync are green. New: `docs/EOL.md`, `/e2eeol`. Full detail below.
