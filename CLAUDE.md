...
- **GitHub Dependabot integration** — new CLI `dependabot-alerts` command pulls open HIGH/CRITICAL Dependabot alerts (GitHub token via `--github-token`/`GITHUB_TOKEN`, fetched with the JDK `HttpClient`, cursor-paginated via the `Link` header) for `--org` and/or `--repos`, groups them by repository, and POSTs to `POST /api/github/dependabot/import` (ADMIN/VULN). The asset is a **repository, not an EC2**: `Asset.type = AssetTypes.REPOSITORY` (new `com.secman.constants.AssetTypes`), `name = "owner/repo"`, `uri = repo URL`, `owner = AssetOwners.GITHUB_IMPORT` ("GitHub Dependabot"). `GitHubDependabotImportService` uses the same transactional delete-insert replace pattern as CrowdStrike (explicit `deleteByAssetId`, NO cascade; `firstSeenAt` preserved per (CVE/GHSA, product)); vulns are tagged `source = VulnerabilitySources.GITHUB_DEPENDABOT`. No schema migration — `Asset.uri`/`type`/`owner` and `Vulnerability.source` already existed. Notifications: the AWS notify flow is hardwired to `cloudAccountId`, so repository vulns go through a parallel `RepositoryVulnerabilityNotificationService` (recipients = workgroup members of the repo asset via `AssetRepository.findDistinctWorkgroupMemberEmailsByAssetId` + owner-as-email), surfaced by CLI `send-repository-notifications` → `POST /api/cli/repository-vulnerability-notifications/send` (ADMIN). Docs: `docs/CLI.md`.
