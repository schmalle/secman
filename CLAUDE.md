...
- **Vulnerability export IP addresses** — the Excel export always had an `IP` column (`ExportJobService`, column 1, from `Asset.ip`); it read blank because ingest never asked CrowdStrike for the IP. All four Spotlight vulnerability queries requested `facet=cve` only, and Spotlight omits the whole `host_info` object — `local_ip`, `machine_domain`, `instance_id`, `service_provider_account_id`, `os_version` — unless `facet=host_info` is asked for, so every mapper read NULLs and the import stored assets with no IP instead of failing (same failure mode as the Discover `facet=install_usage` gap). Also, per-host batches took `firstVuln?.ip`; a shared `resolveHostIp()` now takes the first non-blank IP across the host's rows. Existing assets fill in on the next import via `updateAsset`; no export-side change was needed.
