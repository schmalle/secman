...
- **AWS account risk assessment: validation pass, three defects closed** — the opt-in auto-start path (`--start-risk-assessment` / `--risk-usecase`, default off, mirrored on `import-s3`, REST bulk and MCP `import_user_mappings`) holds, but three things did not. The **deadline had no upper bound**: past ~year 9999 the `endDate` leaves the SQL `DATE` range, and since assessments start *after* the import commits that failed per account with mappings already persisted — now `MAX_DEADLINE_DAYS = 3650` in `validateStartRequest` (shared by REST + MCP), mirrored in both CLI commands, clamped on a direct service call. The **idempotent skip was reported as a failure**: "an open risk assessment already exists" came back in `error`, so the CLI printed ❌ and exited 1 for the feature working correctly — now `skipped`/`skipReason`, with the owner-notification gate testing `!skipped` (it relied on `error != null` to suppress the duplicate mail). The **email boundary was too loose**: `[^@]+@[^@]+\.[^@]+` accepts CR/LF and commas, and that address becomes an SMTP recipient, a log line and the assessment notes — tightened, length-capped at the column width, echoed values sanitized. `/aws-account-risk-assessment` gained opt-in-default, deadline-bound and real-skip-path phases, and its cleanup no longer deletes any assessment whose notes merely contain `886` (it keys on the `e2e-awsra-` owner prefix) and finally removes the `AWS_ACCOUNT` assets it leaked every run. **Kotlin changes are compile-unverified** — no Gradle distribution or Maven Central route in that session. Full detail below.
