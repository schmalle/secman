...
> `.agents/skills/importtest/SKILL.md` are one skill kept in two harness trees
...
1b. Kill any running backend/frontend (./scripts/stopbackenddev.sh +
...
- **Fresh services always**: killing any running backend/frontend via the
  stop scripts and starting both fresh is an unconditional part of Phase 1
  — never attach the import run to services you did not start in this
  invocation. Only resort to `kill -9` after a graceful stop via the stop
  scripts has been attempted and reported.
