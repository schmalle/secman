...
> `.agents/skills/humanizer/SKILL.md` are one skill kept in two harness trees
...
**Propose, then wait** — ask the user (in Claude Code, `AskUserQuestion`) with the call-site
...
Bash tool `dangerouslyDisableSandbox: true`), then stop it. Compile-clean is
